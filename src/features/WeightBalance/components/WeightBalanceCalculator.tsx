...
import { FiAlertTriangle, FiActivity, FiRefreshCw, FiUsers, FiInfo, FiDroplet, FiBox } from 'react-icons/fi';
...
export const WeightBalanceCalculator: React.FC<WeightBalanceCalculatorProps> = ({
  ...
  const [selectedProfileId, setSelectedProfileId] = useState<string | null>(preselectedProfileId || null);
  const [selectedAircraftId, setSelectedAircraftId] = useState<string | null>(preselectedAircraftId || null);

  // Auto-select aircraft from preselected profile
  useEffect(() => {
    if (preselectedProfileId && !preselectedAircraftId) {
      const profile = profiles.find((p: WeightBalanceProfileDto) => p.id === preselectedProfileId);
      if (profile?.aircraftId) {
        setSelectedAircraftId(profile.aircraftId);
      }
    }
  }, [preselectedProfileId, preselectedAircraftId, profiles]);
  ...
  }, [selectedProfile, initializeFromProfile]);
